    ...
    if not api_order:
        ...
        if current_order and current_order.get("amount_irt"):
            ...
            await bot.send_message(
                user_id,
                f"❌ <b>موجودی شماره در لحظه آخر به پایان رسید!</b>\n\n"
                f"کاربر گرامی، از آنجا که موجودی شماره‌های مجازی در سرورهای جهانی به‌صورت لحظه‌ای تغییر می‌کند، دقیقاً در زمان پرداخت شما، این شماره توسط شخص دیگری در شبکه خریداری شد و موجودی آن موقتاً صفر شد.\n\n"
                f"✅ <b>جای نگرانی نیست!</b>\nمبلغ <b>{refund_amount:,} تومان</b> پرداختی شما به‌صورت کامل و اتوماتیک به <b>کیف پول شما در ربات</b> بازگشت داده شد.\n"
                f"هم‌اکنون می‌توانید بدون نیاز به درگاه، از موجودی کیف پول خود برای خرید کشور دیگری استفاده کنید.",
